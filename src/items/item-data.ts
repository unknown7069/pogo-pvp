...
type ItemCategory = 'candy' | 'tm' | 'evolution' | 'premium';
...
interface ItemCatalog {
  list: ItemDefinition[];
  byKey: Record<ItemKey, ItemDefinition>;
  tmKeys: ItemKey[];
  evolutionKeys: ItemKey[];
}
...
const itemByKey = ITEM_DEFINITIONS.reduce<Record<ItemKey, ItemDefinition>>((acc, def) => {
  acc[def.key] = def;
  return acc;
}, {} as Record<ItemKey, ItemDefinition>);
