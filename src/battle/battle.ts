...
(function(){
  ...
  // ---------------------------
  function setHp(barEl: HTMLElement | null, pct: number) {
    if (!barEl) return;
  ...
  function setEn(barEl: HTMLElement | null, pct: number) {
    if (!barEl) return;
  ...
  const forfeitBtn = $('forfeitBtn') as HTMLButtonElement | null;
  const overlay = $('forfeitOverlay') as HTMLElement | null;
  const cancelForfeit = $('cancelForfeit') as HTMLButtonElement | null;
  const confirmForfeit = $('confirmForfeit') as HTMLButtonElement | null;
  ...
  // Countdown overlay
  const cdOverlay = $('countdownOverlay') as HTMLElement | null;
  const cdText = $('countdownText') as HTMLElement | null;
  // End fade overlay
  const fadeOverlay = $('fadeOverlay') as HTMLElement | null;
  ...
  function sanitizeCollectionEntry(entry: unknown) {
    if (!entry || typeof entry !== 'object') return null;
    const src = entry as PokemonCollectionEntry;
    const uid = typeof src.uid === 'string' && src.uid ? src.uid : null;
    const id = Number((src as { id?: unknown }).id);
    const level = sanitizeLevelValue(src.level, 20);
    if (!uid || Number.isNaN(id) || Number.isNaN(level)) return null;
    const normalized: PokemonCollectionEntry & { uid: string; id: number; level: number; name: string | null } = {
      uid,
      id,
      level,
      name: typeof src.name === 'string' ? src.name : null,
    };
    if (src.shiny != null) normalized.shiny = src.shiny;
    if (src.ivHp != null) normalized.ivHp = src.ivHp;
    if (src.ivAttack != null) normalized.ivAttack = src.ivAttack;
    if (src.ivDefense != null) normalized.ivDefense = src.ivDefense;
    ...
    if (src.fastMoveId) normalized.fastMoveId = src.fastMoveId;
    if (Array.isArray(src.chargedMoveIds)) normalized.chargedMoveIds = src.chargedMoveIds.slice();
    if (src.secondChargedMoveId) normalized.secondChargedMoveId = src.secondChargedMoveId;
    return normalized;
  }

  function makePokemonFromId(id: number, levelOverride?: number | null, overrides?: PokemonCollectionEntry | null) {
    const key = Number(id);
  ...
  (function resetStateOnFirstLoad(){
    try {
      ...
      const navType = navEntry && typeof (navEntry as PerformanceNavigationTiming).type === 'string'
        ? (navEntry as PerformanceNavigationTiming).type
        : null;
  ...
  const oppNameEl = document.getElementById('opponent-name') as HTMLElement | null;
  const oppTypesEl = document.getElementById('opponent-types') as HTMLElement | null;
  const oppCpEl = document.getElementById('opponent-cp') as HTMLElement | null;
  const playerNameEl = document.getElementById('player-name') as HTMLElement | null;
  const playerTypesEl = document.getElementById('player-types') as HTMLElement | null;
  const playerCpEl = document.getElementById('player-cp') as HTMLElement | null;
  const oppSpriteEl = document.querySelector('.sprite.opponent') as HTMLElement | null;
  const playerSpriteEl = document.querySelector('.sprite.player') as HTMLElement | null;
  const oppSpriteImg = document.getElementById('opponent-sprite-img') as HTMLImageElement | null;
  const playerSpriteImg = document.getElementById('player-sprite-img') as HTMLImageElement | null;

  // Build PokemonDB sprite URLs from centralized helper
  const oppEffectSlot = document.getElementById('opponent-effect-slot') as HTMLElement | null;
  const playerEffectSlot = document.getElementById('player-effect-slot') as HTMLElement | null;

  function renderTypes(container: HTMLElement | null, types: readonly string[] | null | undefined) {
    if (!container) return;
    container.innerHTML = '';
    (types || []).forEach((t) => {
  ...
  async function performSwitch(nextIndex) {
    ...
    const loadNextSprite = new Promise<void>((resolve) => {
      if (!nextSpriteUrl) { resolve(undefined); return; }
    ...
    const throwBall = (): Promise<void> => new Promise<void>((resolve) => {
      const ballUrl = ballSpriteUrls[0];
      if (!stageEl || !spriteHost || !ballUrl) { resolve(undefined); return; }
      const stageRect = stageEl.getBoundingClientRect();
      const spriteRect = spriteHost.getBoundingClientRect();
      if (!stageRect || !spriteRect) { resolve(undefined); return; }
